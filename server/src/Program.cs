...
if (builder.Environment.IsDevelopment())
{
    var apiKeySection = builder.Configuration.GetSection(ApiKeySettings.SectionName);
...
var environmentName = builder.Configuration["ASPNETCORE_ENVIRONMENT"] ?? builder.Environment.EnvironmentName;
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    c.AddServer(new() { Url = "http://localhost:5166", Description = "Development server" });
    ...
    c.AddSecurityRequirement(doc =>
    {
        ...
        var schemeRef = new OpenApiSecuritySchemeReference("ApiKey", doc);
...
v1.MapPost("/register", async (RegisterRequest request, AppDbContext db) =>
{
    ...
    var deviceId = RequestValidator.Sanitize(request.DeviceId);
    ...
    var existingUser = await db.Users.FirstOrDefaultAsync(u => u.DeviceId == deviceId);
    if (existingUser != null)
    {
        ...
        var userActivities = await db.DailyActivities
            .Where(a => a.UserId == existingUser.Id)
            .ToListAsync();
        var (currentStreak, longestStreak) = calculator.CalculateStreak(userActivities, today);
        ...
        if (currentStreak == 1 && userActivities.Count > 1)
        {
            ...
            {
                var lastActivityDate = previousActivities.First().Date;
                ...
                {
                    var gapDays = today.DayNumber - lastActivityDate.DayNumber;
    ...
    do
    {
        ...
        var codeExists = await db.Users.AnyAsync(u => u.PairCode == pairCode);
        isUnique = !codeExists;
        attempts++;
    ...
    var newUserActivity = new DailyActivity
    {
        UserId = newUser.Id,
        Date = today,
        UpdatedAt = DateTime.UtcNow
    ...
    var (newUserCurrentStreak, newUserLongestStreak) = calculator.CalculateStreak(new List<DailyActivity> { newUserActivity }, today);
    ...
    .Produces<Heartbeat.Contracts.ProblemDetails>(StatusCodes.Status400BadRequest)
    .Produces<Heartbeat.Contracts.ProblemDetails>(StatusCodes.Status401Unauthorized);
